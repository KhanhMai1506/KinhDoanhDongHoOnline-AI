...
async def chat_stream(req: ChatRequest, request: Request):
    ...
    try:
        ...
        if "tôi cần tư vấn" in question.lower():
        ...
        if found_brand and (len(question.split()) <= 4 or "sản phẩm" in question.lower()):
            # Điều chỉnh câu hỏi để RAG tìm kiếm tốt hơn
            question = f"Thông tin chi tiết về các sản phẩm đồng hồ {found_brand}"
            logger.info(f"Optimized question for brand lookup: {question}")
